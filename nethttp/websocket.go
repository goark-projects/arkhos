...
func WebSocketHandler(
	...
	return servlet.HandlerFunc(
		func(ctx context.Context, req *servlet.Request, res servlet.Response) error {
			_, err := servletwebsocket.Upgrade(
				ctx,
				req,
				res,
				servletwebsocket.EndpointHandler(sessionID, endpoint),
				options...)
			return err
		},
	)
...
func WebSocketUpgrade(
	...
	handler servletwebsocket.Handler,
	options ...websocket.HandshakeOption,
...
func WebSocketEndpointHandler(
	sessionID string,
	endpoint websocket.Endpoint,
	options ...servletwebsocket.FrameConnectionOption,
) servletwebsocket.Handler {
	return servletwebsocket.HandlerFunc(
		func(ctx context.Context, handshake websocket.Handshake, conn upgrade.Connection) error {
			return servletwebsocket.ServeEndpoint(
				ctx,
				sessionID,
				handshake,
				conn,
